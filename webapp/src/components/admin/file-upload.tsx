...
export default function ImageUpload() {
    ...
    const getFileIconAndColor = (file: File) => {
        if (file.type.includes(FileTypes.Xml)) {
            return {
                icon: <File size={40} className={XmlColor.fillColor} />,
                color: XmlColor.bgColor,
            };
        }
    ...
    const onDrop = useCallback(async (acceptedFiles: File[]) => {
        ...
        const filePath = path.join(process.cwd(), 'uploads', `test.xml`);
        console.log("KESINI")
        try {
            ...
            if (isValid) {
                console.log("VALID")
                // fs.writeFileSync(filePath, fileContents); // Write file to uploads directory
                // setUploadStatus('File uploaded successfully!');
            } else {
                console.log("NOT VALID")
                // setUploadStatus('Error: Invalid XML format.');
            }
        } catch (error) {
